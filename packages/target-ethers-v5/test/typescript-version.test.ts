...
describe('Ethers target constructor', () => {
  ...
  it('panics when TypeScript version is lower than 4.3', () => {
    ...
    }).toThrow('@gnosis-guild/typechain-ethers-v5 1.2.3 needs TypeScript version 4.3 or newer.')

    const message = consoleErrorMock.calls[0].args[0]

    expect(message).toEqual(
      expect.stringMatching('@gnosis-guild/typechain-ethers-v5 1.2.3 needs TypeScript version 4.3 or newer.'),
    )
    expect(message).toEqual(
